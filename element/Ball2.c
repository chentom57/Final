...
Elements *New_Ball2(int label)
{
    ...
    pDerivedObj->placed_range=New_Circle(pDerivedObj->x,pDerivedObj->y,pDerivedObj->ranges);
...
void Ball2_draw(Elements *self)
{
    ...
    if(Obj->selflw==1||Obj->selflw==2){
        if((Obj->lap)==1)
            al_draw_circle(Obj->x,Obj->y,Obj->ranges,al_map_rgb(255,0,0),10);
        else
           al_draw_circle(Obj->x,Obj->y,Obj->ranges,al_map_rgb(0,255,0),6);
    }
}
